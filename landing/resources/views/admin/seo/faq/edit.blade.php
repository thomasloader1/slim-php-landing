...
@section('header', 'Editar Pregunta')
@section('subheader', 'Modifica esta pregunta frecuente.')
...
<div class="max-w-2xl">
    ...
    <form action="{{ url('admin/seo/faq/edit/' . $item->id) }}" method="POST" class="bg-slate-900 border border-slate-800 p-5 sm:p-8 rounded-3xl shadow-sm space-y-6">
        <div>
        ...
        <div>
            ...
            <textarea name="answer" rows="4" required
                      class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 focus:border-blue-500/50 outline-none transition-all text-white resize-none">{{ $item->answer }}</textarea>
        </div>

        <div>
            <label class="flex items-center gap-3 cursor-pointer group">
                <div class="relative w-12 h-6 bg-slate-800 rounded-full transition-colors group-hover:bg-slate-700 shrink-0">
                    <input type="checkbox" name="active" {{ $item->active ? 'checked' : '' }} class="sr-only peer">
